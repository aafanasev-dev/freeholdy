...
def write_files(project: str, compose_text: str, services: list[dict],
                env_files: dict[str, list[str]] | None = None) -> tuple[str, str]:
    ...
    `env_files` is `env_service.materialize`'s mapping of service name → absolute paths of
    the materialized env files, in precedence order (the shared project-level file first,
    the service's own file second). Compose applies `env_file` entries left to right with
    later entries winning, so that ordering gives service values priority over shared ones.
    Absolute paths keep the files outside the build context and out of version snapshots.
    ...
    override_path = write_override(project, services, env_files)
    return compose_path, override_path


def write_override(project: str, services: list[dict],
                   env_files: dict[str, list[str]] | None = None) -> str:
    """(Re)generate `docker-compose.override.yml` alone and return its path.

    Split out from `write_files` because the override is the only generated artifact that
    depends on the stored environment: a restart rewrites it — so freshly edited variables
    reach the containers — without touching the uploaded compose file or re-provisioning
    ports, subdomains and nginx.
    """
    ...
    for svc in services:
        ...
        paths = (env_files or {}).get(name) or []
        if paths:
            cfg["env_file"] = list(paths)
