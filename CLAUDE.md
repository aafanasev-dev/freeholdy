...
4. Subsequent calls to `app/routers/container.py` (`/projects/{name}/build|start|stop|status|abort|ssl`) operate on the `Project` row via `app/services/docker_service.py`. **Exec is a WebSocket, not REST:** `WS /projects/{name}/exec` bridges an interactive `docker exec -it` shell on a pty (`interactive_service.run_exec_session`; auth → ready → `stdin`/`stdout`/`resize` frames → exit). An optional `?cmd=` overrides the default shell.
